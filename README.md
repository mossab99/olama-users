...
- Temp Families are local WordPress accounts that are never synchronized with OLAMA Core. Administrators explicitly assign their students, may set an expiry date, and can deactivate or reactivate access from the Temp Families submenu.
- Temp Families receive student-level Student Gateway access only. Family profile, finance, transportation, WordPress administration, and unassigned siblings remain unavailable by default.
