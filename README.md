...
- Temp Families are local WordPress accounts that are never synchronized with OLAMA Core. Administrators define local family members, select each member's OLAMA School grade and section, may set an expiry date, and can deactivate or reactivate access from the Temp Families submenu.
- Temp Families sign in with their full `tf_` username. When the suffix is numeric, the portal also accepts the number without `tf_`; an existing Core family number always takes priority.
- Temp Families receive grade/section-based Student Gateway access only. This includes a browser-only demo of published online exams; answers, attempts, marks, and results are never saved. They are not linked to existing Core students; family profile, finance, transportation, WordPress administration, and student-specific records remain unavailable by default.
- The Roles screen allows administrators to create, duplicate, rename, and safely delete custom roles. WordPress Administrator and required system roles are protected. Deletion moves assigned users to Subscriber and clears a matching family or employee import default, locking Apply until a replacement is selected. Deleted roles stay suppressed even when a legacy OLAMA plugin previously created them automatically.
...
- OLAMA Users manages OLAMA role definitions and declared plugin capabilities. WordPress and compatible role-management plugins retain normal ownership of user-role assignments, including multiple roles per user.
